...
class PersonalLogic {
  ...
  */
  HashMap<int, List<String>> readPersonalTasks(List<int> deletedIndexes) {
    int count = 1;
    int deletedTasksSize = deletedIndexes.length;
    int indexSimilarityCount = 0;
    String typeOfGrouping = "";
    List<String> defaultList = ["", "", "", "", ""];
    HashMap<int, List<String>> personalTasks = HashMap();
    ...
    if (tasksDatabase.get(0) != null) {
      ...
      for (int x = 1; x <= currentNumberOfTasks; x++) {
        ...
        if (indexSimilarityCount == 0) {
          ...
          if (typeOfGrouping == "Personal") {
            personalTasks[count] = tasksDatabase.get(x);
            count++;
          }
        }
    ...
    /// execute this if the the value at index 0 is null (initial value is not predefined)
      ...
      personalTasks[1] = defaultList;
    }

    displayToast(personalTasks.toString());
